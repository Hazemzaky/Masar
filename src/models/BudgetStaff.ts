...
export interface IBudgetStaff extends Document {
  year: number;
  no: string;
  description: string;
...
const BudgetStaffSchema = new Schema<IBudgetStaff>({
...
BudgetStaffSchema.index({ year: 1, no: 1 }, { unique: true });

export default mongoose.model<IBudgetStaff>('BudgetStaff', BudgetStaffSchema);
