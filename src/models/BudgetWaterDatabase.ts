...
export interface IBudgetWater extends Document {
  year: number;
  no: string;
  description: string;
...
const BudgetWaterSchema = new Schema<IBudgetWater>({
...
BudgetWaterSchema.index({ year: 1, no: 1 }, { unique: true });

export default mongoose.model<IBudgetWater>('BudgetWater', BudgetWaterSchema);
