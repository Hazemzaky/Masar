...
export interface IBudgetOthers extends Document {
  year: number;
  no: string;
  description: string;
...
const BudgetOthersSchema = new Schema<IBudgetOthers>({
...
BudgetOthersSchema.index({ year: 1, no: 1 }, { unique: true });

export default mongoose.model<IBudgetOthers>('BudgetOthers', BudgetOthersSchema);
