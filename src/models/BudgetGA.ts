...
export interface IBudgetGA extends Document {
  year: number;
  no: string;
  description: string;
...
const BudgetGASchema = new Schema<IBudgetGA>({
...
BudgetGASchema.index({ year: 1, no: 1 }, { unique: true });

export default mongoose.model<IBudgetGA>('BudgetGA', BudgetGASchema);
